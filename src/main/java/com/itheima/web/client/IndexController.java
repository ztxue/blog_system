...
//        System.out.println("allowComment------------>"+article.getAllowComment());
        if(article!=null){
            // 查询封装评论相关数据
    ...
    private void getArticleComments(HttpServletRequest request, Article article) {
        System.out.println("article.getAllowComment()----->"+article.getAllowComment());
    ...
    public String findLike(HttpServletRequest request){
        String title = request.getParameter("title");
        List<Article> titleByLike = articleServiceImpl.findByLike(title);
        request.setAttribute("titleByLike",titleByLike);
        return "client/likeArticle";
    }
}
