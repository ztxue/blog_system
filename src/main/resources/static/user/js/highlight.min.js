...
!function (e) {
    ...
    function i(e) {
        ...
        for (s = s.split(/\s+/), t = 0, a = s.length; a > t; t++) if (i = s[t], n(i) || y(i)) return i
    }
    ...
    function u(e, r, n, i) {
        ...
        function p(e, t, r, a) {
            var n = a ? "" : S.classPrefix, i = '<span class="' + n, s = r ? "" : B;
            return i += e + '">', i + t + s
        }
        ...
        function m() {
            var e, r, a, n;
            if (!N.k) return t(E);
            for (n = "", r = 0, N.lR.lastIndex = 0, a = N.lR.exec(E); a;) n += t(E.substring(r, a.index)), e = b(N, a), e ? (M += e[1], n += p(e[0], t(a[0]))) : n += t(a[0]), r = N.lR.lastIndex, a = N.lR.exec(E);
    ...
    var w = [], N = Object.keys, k = {}, x = {}, C = /^(no-?highlight|plain|text)$/i, E = /\blang(?:uage)?-([\w-]+)\b/i,
        M = /((^(<[^>]+>|\t|)+|(?:\n)))/gm, B = "</span>",
    ...
    return e.highlight = u, e.highlightAuto = d, e.fixMarkup = b, e.highlightBlock = m, e.configure = f, e.initHighlighting = g, e.initHighlightingOnLoad = _, e.registerLanguage = h, e.listLanguages = v, e.getLanguage = y, e.inherit = s, e.IR = "[a-zA-Z]\\w*", e.UIR = "[a-zA-Z_]\\w*", e.NR = "\\b\\d+(\\.\\d+)?", e.CNR = "(-?)(\\b0[xX][a-fA-F0-9]+|(\\b\\d+(\\.\\d*)?|\\.\\d+)([eE][-+]?\\d+)?)", e.BNR = "\\b(0b[01]+)", e.RSR = "!|!=|!==|%|%=|&|&&|&=|\\*|\\*=|\\+|\\+=|,|-|-=|/=|/|:|;|<<|<<=|<=|<|===|==|=|>>>=|>>=|>=|>>>|>>|>|\\?|\\[|\\{|\\(|\\^|\\^=|\\||\\|=|\\|\\||~", e.BE = {
        ...
        var t = "[脌-矢a-zA-Z_$][脌-矢a-zA-Z_$0-9]*", r = t + "(<" + t + "(\\s*,\\s*" + t + ")*>)?",
        ...
        var t = {b: "\\$+[a-zA-Z_-每][a-zA-Z0-9_-每]*"}, r = {cN: "meta", b: /<\?(php)?|\?>/}, a = {
            cN: "string",
            c: [e.BE, r],
